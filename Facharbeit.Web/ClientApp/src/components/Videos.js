...
export class Videos extends Component {
    ...
    render() {
        ...
        return (
            <Layout currentPage={this.state.type == 0 ? "Serien" : "Filme"}>
                {contents}
            </Layout>
        );
    }
                ...
                <div className="lolomoRow lolomoRow_title_card ltr-0">
                    <h2 className="rowHeader ltr-0">
                        <span className="rowTitle ltr-0">
                            <div className="row-header-title">Alle Serien</div>
                        </span>
                    </h2>
    ...
    async getVideos() {
        const token = await authService.getAccessToken();
        console.log(token);
        const response = await fetch(`api/video/getvideosbytype?type=${this.state.type}`, {
            method: 'POST',
...
export default withParams(Videos);